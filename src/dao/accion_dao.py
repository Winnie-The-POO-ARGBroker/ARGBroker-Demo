...
class AccionDAO:
    ...
    def obtener_precio_actual_compra(self, nombre_activo):
        try:
            cursor = self.conexion.cursor(dictionary=True)
            query = "SELECT precio_compra FROM acciones WHERE nombre_empresa = %s"
            cursor.execute(query, (nombre_activo,))
            resultado = cursor.fetchone()

            if resultado:
                return Decimal(resultado['precio_compra'])
            else:
                raise ValueError(f"No se encontró el activo {nombre_activo} en la base de datos.")
    ...
    def obtener_precio_actual_venta(self, nombre_activo):
        try:
            cursor = self.conexion.cursor(dictionary=True)
